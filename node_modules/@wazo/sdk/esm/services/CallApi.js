...
class CallApi {
  ...
  static async cancelCall(server        , session         , callSession             )                {
    return getApiClient(server).ctidNg.cancelCall(session.token, callSession.callId);
  }

  static async makeCall(server        , session         , callFromLine      , extension        )                 {
    if (!callFromLine) {
      return null;
    }
